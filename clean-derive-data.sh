...
usage() {
  cat <<- EOF
  $(bold Usage:) ${script_basename} [OPTION] project-name
  a shell template
...
main() {
	...
	if [[ -z "$PROJECT" ]]; then
 	...
 	#find "$HOME/Library/Developer/Xcode/DerivedData" -name "${PROJECT}*" -depth 1 -exec rm -rf {} \;
 	rm -rf "$HOME/Library/Developer/Xcode/DerivedData/${PROJECT}"*
}
